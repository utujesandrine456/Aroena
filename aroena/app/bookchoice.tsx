...
import { View, Text, StyleSheet, Image, Dimensions, TouchableOpacity, TextInput, ScrollView, ActivityIndicator } from 'react-native';
...
const initialServices = [
    { id:'1', title:'Private Deluxe Room', description:'Spacious private room with view, king bed, modern amenities.', category:'Room', price:120000, priceUnit:'/night', rating:4.8, image:require('../assets/images/room1.jpg'), available:true, features:['Private Bathroom','Free WiFi','Breakfast Included'] },
    { id:'2', title:'Grilled Fried Fish', description:'Fresh fried fish with local spices and sides.', category:'Food', price:15000, priceUnit:'/plate', rating:4.5, image:require('../assets/images/Fish.jpg'), available:true, features:['Fresh Catch','Served Hot','30 Min Delivery'] },
    { id:'3', title:'Personal Standard Room', description:'Comfortable room for solo travelers.', category:'Room', price:18000, priceUnit:'/night', rating:4.7, image:require('../assets/images/room2.jpg'), available:false, features:['Single Bed','Desk Space','Daily Cleaning'] },
    { id:'4', title:'Roasted Meat & Chips', description:'Seasoned roasted meat with crispy chips.', category:'Food', price:25000, priceUnit:'/plate', rating:4.9, image:require('../assets/images/chips.jpg'), available:true, features:['Chef Special','Large Portion','Fast Service'] },
    { id:'5', title:'Standard Double Room', description:'Affordable double room with comfort and service.', category:'Room', price:80000, priceUnit:'/night', rating:4.2, image:require('../assets/images/room3.jpg'), available:true, features:['City View','Work Desk','Room Service'] },
];

const filters = [
  { id: 'All', label: 'All', icon: 'apps' as const },
  { id: 'Room', label: 'Rooms', icon: 'bed' as const },
  { id: 'Food', label: 'Dining', icon: 'restaurant' as const },
  { id: 'Available', label: 'Available', icon: 'checkmark-circle' as const }
];
...
export default function BookChoice() {
  ...
  useEffect(() => {
    setLoading(true);
    let filtered = selectedFilter === 'All' ? [...services] : selectedFilter === 'Available' ? services.filter(s => s.available) : services.filter(s => s.category === selectedFilter);
    if (searchQuery.trim()) filtered = filtered.filter(s => s.title.toLowerCase().includes(searchQuery.toLowerCase()) || s.description.toLowerCase().includes(searchQuery.toLowerCase()) || s.features.some(f => f.toLowerCase().includes(searchQuery.toLowerCase())));
    setTimeout(() => { setFilteredServices(filtered); setLoading(false); }, 300);
    ...
    <View style={styles.container}>
      ...
      <Animatable.View animation="fadeInUp" delay={200} style={styles.filtersContainer}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filtersScroll}>
          {filters.map((filter) => (
            <TouchableOpacity key={filter.id} style={[styles.filterChip, selectedFilter === filter.id ? styles.filterChipSelected : null]} onPress={() => setSelectedFilter(filter.id)} activeOpacity={0.7}>
            ...
            <Animatable.View key={service.id} animation="fadeInUp" delay={300 + idx * 100} style={styles.serviceCard}>
              ...
              <View style={styles.serviceInfo}>
                ...
                <View style={styles.actionRow}>
                  ...
                  <TouchableOpacity style={[styles.bookButton, !service.available ? styles.bookButtonDisabled : null]} onPress={() => handleBookNow(service)} disabled={!service.available} activeOpacity={0.8}>
